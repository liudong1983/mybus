...
int process(bus::row_t *in_row)
{
    ...
    if (schema_id == 0) {
        ...
        /* 获得数据 */
        char *owneruid, *otheruid, *type;
        bool ret1 = in_row->get_value(0, &owneruid);
        bool ret2 = in_row->get_value(1, &otheruid);
        bool ret3 = in_row->get_value(2, &type);
        if (!ret1 || !ret2 || !ret3) {
        ...
        /* 设定目标切片号 */
        char key_buf[128];
        snprintf(key_buf, sizeof(key_buf), "%s_%s", owneruid, otheruid);

        char hash_key[128];
        snprintf(hash_key, sizeof(hash_key), "%s%s", owneruid, otheruid);

        uint32_t dst_partnum = crc32(0, hash_key, strlen(hash_key))&0xffffffff % instance_num;
